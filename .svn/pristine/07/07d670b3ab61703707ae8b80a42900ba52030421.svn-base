...
<?php if (!$noNodesMessage): ?>
    ...
    $dataFile = \frontend\models\ProjectProcessTemplate::find()->select('name')->indexBy('name')->column();
    $projectId = $node->project_id;
    if(empty($node->project_id))//说明是新建的
        $projectId = $parent->project_id;
    //$project = \frontend\models\ProjectProcess::find()->where(['lvl'=>3,'project_id'=>$projectId])->select('name')->indexBy('name')->column();
    $project = \frontend\models\ProjectProcess::find()->where(['project_id'=>$projectId])->select('name')->indexBy('name')->column();
