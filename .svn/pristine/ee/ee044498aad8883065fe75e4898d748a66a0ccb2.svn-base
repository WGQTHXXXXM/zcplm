...
class MaterialsController extends Controller
{
     ...
     * 更改采购推荐级别的页面
     */
    public function actionChangeRecommendPurchase()
    {
        $searchModel = new MaterialsSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        $dataProvider->setPagination(['defaultPageSize'=>10]);
        return $this->render('change-recommend-purchase', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
     ...
     * 保存更改的采购推荐级别
     */
    public function actionSaveRecommendPurchase()
    {
        if (isset($_POST['hasEditable']))
        {
            Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;//这个表格编辑控件需要的格式，把返回的数据变成json
            $model = Materials::findOne($_POST['editableKey']);
            $data = current($_POST['Materials']);//表单数据
            $model->recommend_purchase = $data['recommend_purchase'];
            if($model->save())
     ...
     * 保存更改的采购推荐级别
     */
    public function actionSaveManufacturer()
    {
        if (isset($_POST['hasEditable']))
        {
            Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;//这个表格编辑控件需要的格式，把返回的数据变成json
     ...
     * 没有上传附件的列表
     */
    public function actionNoattach()
    {
        $searchModel = new MaterialsSearch();
        $dataProvider = $searchModel->searchAttach(Yii::$app->request->queryParams);
        return $this->render('noattach', [
            'searchModel' => $searchModel,
            'dataProvider' => $dataProvider,
        ]);
    }
    ...
    public function actionView($id,$modal=0)
    {
        ...
        $tbla = MaterialAttachment::find()->alias('tblmax')->select('material_id,file_class_name,max(version) as maxv')
            ->groupBy('material_id,file_class_name')
            ->where(['material_id'=>$model->material_id])->andWhere('modify_material_id<>-1');
        $dataAttachmentOld = new ActiveDataProvider([
            'query' => MaterialAttachment::find()->alias('ma')
                ->rightJoin(['tbla'=>$tbla],'ma.version=tbla.maxv and ma.file_class_name=tbla.file_class_name and ma.material_id=tbla.material_id')
        ]);
    ...
    public function actionSyncMfr()
    {
        ...
        foreach ($tables as $tbl){
            ...
            if(empty($datas[0]))
                continue;
            $sysMtr = Materials::findOne(['zc_part_number'=>$datas[0]->zc_part_number]);
            if(empty(MaterialEncodeRule::findOne($sysMtr->manufacturer)))
            {
                var_dump($sysMtr);die;
            }
            $mfrs = MaterialEncodeRule::findOne($sysMtr->manufacturer)->getFather()->getSon();
            $arrMfr = [];
            foreach ($mfrs as $val){
                $arrMfr[$val->id] = $val->name;
            }
            foreach ($datas as $zcmtr){
                $count++;
                $sysMtr = Materials::findOne(['zc_part_number'=>$zcmtr->zc_part_number]);
                if($arrMfr[$sysMtr->manufacturer] != $zcmtr->Manufacturer){
                    var_dump($i.'-->'.$tbl.'----->'.$zcmtr->Manufacturer);
                    $zcmtr->Manufacturer = $arrMfr[$sysMtr->manufacturer];
                    $zcmtr->save();
                    $i++;
                }
            }
     ...
     * 同步candence库和系统库二供
     */
    public function actionSyncProv2()
    {
        $tables = $this->tableName;
        unset($tables['pcba']);
        unset($tables['pcb']);
        ...
        foreach ($tables as $key =>$tbl){
            ...
            if(empty($datas[0]))
                ...
                foreach ($datas as $zcmtr){
                    $count++;
                    if(!empty($zcmtr->getAttribute('2zc_part_number'))){
                        if(!empty($zcmtr->getAttribute('2zc_part_number'))){
                            $sysMtr = $tbl::findOne(['zc_part_number'=>$zcmtr->getAttribute('2zc_part_number')]);
                            if(empty($sysMtr)) {
                                $mmtr = Materials::findOne(['zc_part_number'=>$zcmtr->getAttribute('2zc_part_number')]);
                                $partType = MaterialEncodeRule::findOne($mmtr->part_type);
                                if(!$this->syncMaterialaa($tables[strtolower($key)],$mmtr,true,$partType->name))
                                {
                                    var_dump(123);
                                    $transaction->rollBack();die;
                                }
                                var_dump($i.'---->'.$zcmtr->getAttribute('2zc_part_number'));
                                $i++;
                            }
//                            if($sysMtr->getAttribute('Manufacturer') != $zcmtr->getAttribute('2Manufacturer')){
//                                var_dump($i.'-->'.$tbl.'----->'.$zcmtr->Manufacturer);
//                                $zcmtr->setAttribute('2Manufacturer',$sysMtr->getAttribute('Manufacturer'));
//                                $zcmtr->save();
//                                $i++;
//                            }
//                            if($sysMtr->getAttribute('Mfr_part_number') != $zcmtr->getAttribute('2Mfr_part_number')){
//                                var_dump($i.'-->'.$tbl.'----->'.$zcmtr->Manufacturer);
//                                $zcmtr->setAttribute('2Mfr_part_number',$sysMtr->getAttribute('Mfr_part_number'));
//                                $zcmtr->save();
//                                $i++;
//                            }
//                            if($sysMtr->getAttribute('Description') != $zcmtr->getAttribute('2Description')){
//                                var_dump($i.'-->'.$tbl.'----->'.$zcmtr->Manufacturer);
//                                $zcmtr->setAttribute('2Description',$sysMtr->getAttribute('Description'));
//                                $zcmtr->save();
//                                $i++;
//                            }
                        }
    ...
    public function syncMaterialaa($tableName,$model,$isCreate,$partTypeName)
    {
        $className = $tableName;//插入的是哪个表
        $classNcName = $className."Nc";
        //更新还是插入
        ...
        if(!$hardwareModel->save())
        {
            var_dump($hardwareModel->getErrors());
            return false;
        }
        ...
        if(!$hardwareNcModel->save())
        {
            echo $hardwareNcModel->getErrors();
            return false;
        }
        return true;
    }
     ...
     * 同步candence库和系统库二供
     */
    public function actionSyncProv1()
    {
        $tables = $this->tableName;
        unset($tables['pcba']);
        unset($tables['pcb']);
                ...
                if(!empty($zcmtr->getAttribute('2zc_part_number'))){
                    if(!empty($zcmtr->getAttribute('2zc_part_number'))){
                        $sysMtr = $tbl::findOne(['zc_part_number'=>$zcmtr->getAttribute('2zc_part_number')]);
                            ...
                            if($sysMtr->getAttribute('Manufacturer') != $zcmtr->getAttribute('2Manufacturer')){
                                var_dump($i.'-->'.$tbl.'----->'.$zcmtr->zc_part_number);
                                $zcmtr->setAttribute('2Manufacturer',$sysMtr->getAttribute('Manufacturer'));
                                $zcmtr->save();
                                $i++;
                            }
