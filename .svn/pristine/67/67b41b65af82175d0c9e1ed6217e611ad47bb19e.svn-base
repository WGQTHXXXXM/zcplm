...
//$this->title = ProjectProcess::findOne($project_id)->name . ': ' . ProjectProcess::findOne($file_id)->name;
...
<div class="file-attachments-view">
    ...
    <?= GridView::widget([
      ...
      //  'pjax' => true, //为true时文件下载得用右键选择
        ...
        'toggleData' => false,
      //  'hover'=>true,
        'panel' => ['type' => 'default', 'footer' => false],
        'columns' => [
            ...
            [
                ...
                /*  'buttons' => [
                      'delete' => function ($url, $model, $key) {
                          if (Yii::$app->user->id === $model->fileAttachment->submitter->id) {
                              return Html::a('<span class="glyphicon glyphicon-trash"></span>', ['delete', 'file_attachment_id' => $model->file_attachment_id], [
                                  'title' => '删除',
                                  'data' => [
                                      'confirm' => Yii::t('common', 'Are you sure you want to delete this item?'),
                                      'method' => 'post',
    ...
    <?php $form = ActiveForm::begin([
        'action' => ['file-attachments/upload', 'project_id' => $project_id, 'file_id' => $file_id],
        'method' => 'post',
