...
/**
 * 功能：一个ECN有可能对应多个“变更集合”ecn_change_set。
...
class EcnChangeSet extends \yii\db\ActiveRecord
{
     ...
     * 手动保存变更集合
     */
    public static function saveChangeSet($ecnId,$serial_number)
    {
        $dataPost = $_POST['Ecn'];
        if(!isset($dataPost['change_type']))
            return true;
        foreach ($dataPost['change_type'] as $key=>$type)
        {
            ...
            //看是否与ECN编号相同（有可能编辑时间过长，别人已经生成两个ECN,这时就会不准）
            if($serial_number!=substr($dataPost['change_sn'][$key],0,16))
                $dataPost['change_sn'][$key] = $serial_number.substr($dataPost['change_sn'][$key],16);
