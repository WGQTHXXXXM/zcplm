...
if ($noNodesMessage) {
    ...
    $parent = $node->parents(1)->andWhere(['project_id'=>$node->project_id])->one();
...
var inputCarNumber = $('input[name="ProjectMaterial[car_number]"]');
...
var inputChangeFun = function() {
    ...
    url = url+'&ProjectMaterial[remark]='+$('input[name="ProjectMaterial[remark]"]').val()+
        '&ProjectMaterial[car_number]='+$('input[name="ProjectMaterial[car_number]"]').val()+
    ...
    $.post(url,{},
        function(obj) {
            ...
            inputCarNumber.on('change',inputChangeFun);
            ...
            inputCarNumber.on('keydown',function(e) {
              e = e || event;
              if(e.keyCode == 13){
                inputChangeFun(); 
              }
