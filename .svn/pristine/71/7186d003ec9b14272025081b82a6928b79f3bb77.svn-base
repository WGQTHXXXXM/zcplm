...
class EcrAndEcn extends \yii\db\ActiveRecord
{
     ...
     * 添加ECN的信息
     */
    public function addEcnDate()
    {
        $mdlEcn = Ecn::findOne(['ecr_id'=>$this->id]);
        $this->change_now = $mdlEcn->change_now;
        $this->affect_stock = $mdlEcn->affect_stock;
        $this->remark = $mdlEcn->remark;
    }
    ...
    public function getApprovers()
    {
        ...
        $dataUser[2] = Approver::getApprovers(Approver::TYPE_LEADERS_ECR);
    ...
    public function saveApprover($update=false)
    {
        ...
        if($type == EcApproval::TYPE_ECR)
            $modelApprove->approver3 = Approver::TYPE_LEADERS_ECR;
        else
            $modelApprove->approver3 = Approver::TYPE_LEADERS_ECN;
