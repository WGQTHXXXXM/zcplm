...
//        var_dump($model->find()->select('remark as AA,id')->all() );die;
        ...
        //自己所有的任务都显示
        $dataProvider->query->andFilterWhere(["tasks.user_id"=>Yii::$app->user->id]);
    ...
    public function actionChangeUser()
    {
        if (isset($_POST['hasEditable']))
        {
            ...
            $mail = User::findOne($data['user_id'])->email;
            CommonFunc::sendMail(CommonFunc::TASK_CHANGE_USER,$mail,$mdlTasks->name,'');
        ...
        else if($mdlTasks->getOldAttribute('status') == Tasks::STATUS_REJECTED)//如果之前是被退回
        {
                ...
                ||$mdlTasks->type == Tasks::TASK_TYPE_MTR_APPROVER2)
            {
                ...
                if(strpos($mdlTasks->name,'新增物料')!==false)//把以前的二审三审变回到一审
                    $mdlTasks->name = ModifyMaterial::MATERIAL_CREATE1;
                else
                    $mdlTasks->name = ModifyMaterial::MATERIAL_UPDATE;
                //找到更新后的审批人
