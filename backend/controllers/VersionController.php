...
class VersionController extends Controller
{
    ...
    {
        $allUser = User::find()->select('email')->where('status!=0')->column();
        $maxId = Version::find()->groupBy('id')->select('max(id) as id')->orderBy(['id'=>SORT_DESC])->one()->id;
        $content = Version::findOne($maxId)->content;
        $content = str_replace("\n","<br>",$content);
        CommonFunc::sendMail(CommonFunc::PLM_UPGRADE_NOTICE,$allUser,$content,1);
        echo '发送成功';
    }
