        ...
        <?php else: ?>
            <div class="row">
                ...
                <div class="col-sm-3">
                    <?php
                    //部门
                    $arrDpt = \backend\models\Department::find()->select('name,id')->indexBy('id')->column();
                    $arrDpt[0] = '没有部门';
                    echo $form->field($node, 'department_id')->dropDownList($arrDpt)->label('文件所属部门')
                    ?>
                </div>
            </div>
            <br><br>
...
///////////////////审批部门设定/////////////////////////////
//点击的是文件     或新建
if ($node->lvl == 3||(isset($parent)&&!empty($parentKey)&&$parent->lvl==2)) {//只有点文件时才会显示
