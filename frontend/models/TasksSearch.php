...
class TasksSearch extends Tasks
{
    ...
    public function search($params)
    {
        ...
        if(!empty($this->name))
        {
            ...
            //物料文件上传
            $tempMtrAttach = MaterialAttachment::find()->where(['like','name',$tempName])->select('id')->column();
            if(!empty($tempMtrAttach)) {
                if(empty($tempMtr)&&empty($tempBomsParent)&&empty($tempEcr)&&empty($tempEcn))
                    $query->andFilterWhere(['and',['type'=>Tasks::TASK_TYPE_MTR_FILE_UPLOAD],['in', 'type_id',$tempMtrAttach]]);
                else
                    $query->orFilterWhere(['and',['type'=>Tasks::TASK_TYPE_MTR_FILE_UPLOAD],['in', 'type_id',$tempMtrAttach]]);
            }
            else
                $query->andFilterWhere(['<>','type',Tasks::TASK_TYPE_MTR_FILE_UPLOAD]);

            //质量体系管理文件上传
            $tempQsmAttach = QsmAttachment::find()->where(['like','name',$tempName])->select('id')->column();
            if(!empty($tempQsmAttach)) {
                if(empty($tempMtr)&&empty($tempBomsParent)&&empty($tempEcr)&&empty($tempEcn)&&empty($tempMtrAttach))
                    $query->andFilterWhere(['and',['type'=>Tasks::TASK_TYPE_QSM],['in', 'type_id',$tempQsmAttach]]);
                else
                    $query->orFilterWhere(['and',['type'=>Tasks::TASK_TYPE_QSM],['in', 'type_id',$tempQsmAttach]]);
            }
            else
                $query->andFilterWhere(['<>','type',Tasks::TASK_TYPE_QSM]);
