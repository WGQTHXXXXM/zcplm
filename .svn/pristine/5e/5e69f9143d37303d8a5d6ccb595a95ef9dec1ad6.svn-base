...
//            ->innerJoin('ecn_part_user','ecn_part_user.ecn_change_id=ecn_change_set.id')->column();
        ...
        MaterialAttachment::deleteAll(['in','material_id',$arrDelPbom]);
...
////////////////////以下是ECN任务已通过时，对bom的编辑/////////////////////////////////////
     ...
     * ecn任务通过时
     */
    public static function approveEcn()
    {
        $timeStp = time();
...
//            ->andWhere(['pv_expire_date'=>BomsParent::EXPIRE_DATE_TEMPORARY])
//            ->select('id')->column();
        if(BomsParent::updateAll(['pv_expire_date'=>$timeStp,'status'=>self::STATUS_FORBIDDEN],
                ['pv_expire_date'=>self::EXPIRE_DATE_TEMPORARY])===false)
            return false;
...
//        $arrDelBom = EcnBomidTmp::find()->where(['ecn_id'=>$idEcn])->select('bom_id')->column();
    ...
    public function generateUpBomMtr($ecnId,$needNew,$mtrPcb,&$cache)
    {
        ...
        foreach ($allCboms as $cbom){
            ...
            if (array_search($mdlPbom->parent_id,$cache) === false){//如果这次变更，之前有新建过Boms_parent,这次就不建了
                array_push($cache,$mdlPbom->parent_id);
                ...
                //升级$cbom和mdlparent
                if(!$this->generatePCbomByNewMtr($cbom,$mdlParent,$newCmtr->material_id,$newPmtr->material_id,$ecnId,true ))
                    return -2;
            }else{
                $maxVersion = self::getBomHighestVersion($mdlPbom->parent_id);
                $mdlParent = ExtBomsParent::findOne(['parent_id'=>$mdlPbom->parent_id,'parent_version'=>$maxVersion]);
                $newPmtr = Materials::findOne($mdlParent->real_material);
                //升级$cbom和mdlparent
                if(!$this->generatePCbomByNewMtr($cbom,$mdlParent,$newCmtr->material_id,$newPmtr->material_id,$ecnId,false))
                    return -2;
            }

            if(!$mdlParent->generateUpBomMtr($ecnId,false,'',$cache))
                return -3;
        }
        return $newCmtr;
    }
     ...
     */
    public function generatePCbomByNewMtr(BomsChild $cbom,BomsParent $pbom,$child_id,$real_material,$ecnId,$isGeneratePbom)
    {
        $time = time();
        if($isGeneratePbom){//创建过了，就不建了。
            //创建新的Boms_parent
            ...
            $newPbom->type = self::BOM_TYPE_TRIAL;
    ...
    private function generaUpgradeMtr($needNew,$mtrPcb)
    {
        ...
        $upgradeZcPartNo = self::upgradeZcPartNo($oldMtr->zc_part_number);//升级的物料号
        ...
        if($needNew == true)
        {
            ...
            $newMtr->date_entered = date('Y-m-d H:i:s',time());
            $newMtr->material_id=null;
            if(empty($mtrPcb))//如果为空正常，不为空说明有pcb更换，新建的物料要按pcb的料号建
                $newMtr->zc_part_number = $upgradeZcPartNo;
            else{
                $mtrPcb[0]='P';
            ...
            $mdlMtrFiles = MaterialAttachment::find()->where(['material_id'=>$oldMtr->material_id])->all();
            $time = time();
            foreach ($mdlMtrFiles as $mdlMtrFile){
                $newMtrFile = new MaterialAttachment();
                $newMtrFile->attributes = $mdlMtrFile->attributes;
                $newMtrFile->version = 1;
                $newMtrFile->modify_material_id = 0;
                $newMtrFile->updated_at = $time;
                $newMtrFile->material_id = $newMtr->material_id;
                if(!$newMtrFile->save())
                    return false;
            }
            return $newMtr;
        ...
        return Materials::findOne(['zc_part_number'=>$upgradeZcPartNo]);
    }

    /*
     * 生成智车料号
     */
    public static function upgradeZcPartNo($oldZcPartNo)
    {
